...
class rbNode( bt.Node ):
    ...
    def __init__( self, key=None, color=Color.BLACK, left=None, right=None ):
        """Initialize a new Red-Black tree node: it is a BinaryTree node, with an extra Color attribute.

        :param key: the value to be stored (default: none)
        :type key: int, str, object...
...
############# QUERY METHODS #########################
    ...
    def list(self):
        return bt.BinaryTree( array=self.to_array()).list()
